...
impl TextInput {
    ...
    pub(crate) fn bind_keys(cx: &mut App) {
        let context = Some("TrackNameInput");
        cx.bind_keys([
...
impl EntityInputHandler for TextInput {
    ...
    fn replace_and_mark_text_in_range(
        ...
        self.selected_range = selected_range
            .as_ref()
            .map(|selection| {
                range.start + utf16_to_utf8_offset(&text, selection.start)
                    ..range.start + utf16_to_utf8_offset(&text, selection.end)
            })
...
impl Element for TextElement {
    ...
    fn paint(
        ...
        line.paint(bounds.origin, window.line_height(), window, cx)
            .expect("input line should paint");
...
#[cfg(test)]
mod tests {
    use super::{utf8_to_utf16_offset, utf16_to_utf8_offset};

    #[test]
    fn converts_ime_ranges_between_utf16_and_utf8() {
        let text = "音声🎙️ A";
        for utf8 in text
            .char_indices()
            .map(|(index, _)| index)
            .chain(std::iter::once(text.len()))
        {
            ...
            assert_eq!(utf16_to_utf8_offset(text, utf16), utf8);
        }
    }
}
